...
namespace CSX
{
    public class CSX : BaseSettingsPlugin<CSXSettings>
    {
        ...
        private void HandleCustomSkill(string skillId)
        {
            ...
            if (!IsSkillReady(skillCfg, skillId)) return;
            if (IsPlayerBusy())
            {
                if (!_pendingCasts.Contains(skillId)) _pendingCasts.Enqueue(skillId);
                return;
            }
        ...
        {
            int enemiesCount = CountEnemies(skillCfg.TriggerRange.Value);
            int alliesCount = CountAllies(skillCfg.TriggerRange.Value);
            ...
            if (!enemyOk || !allyOk) return false;
            ...
            if (!_lastUsedTime.ContainsKey(dictKey)) _lastUsedTime[dictKey] = DateTime.MinValue;
            double elapsedMs = (DateTime.Now - _lastUsedTime[dictKey]).TotalMilliseconds;
            if (elapsedMs < skillCfg.CustomCooldownMs.Value) return false;
            return true;
        }
        ...
        private int CountEnemies(int range)
        {
            var localPlayer = GameController.Game.IngameState.Data.LocalPlayer;
            var myPos = new Vector2(localPlayer.Pos.X, localPlayer.Pos.Y);
            int count = 0;
            foreach (var monster in _enemyEntities)
            {
                var monPos = new Vector2(monster.Pos.X, monster.Pos.Y);
                if (Vector2.Distance(myPos, monPos) <= range) count++;
            }
            return count;
        }

        private int CountAllies(int range)
        {
            var localPlayer = GameController.Game.IngameState.Data.LocalPlayer;
            var myPos = new Vector2(localPlayer.Pos.X, localPlayer.Pos.Y);
            int count = 0;
            foreach (var ally in _alliedPlayers)
            {
                var allyPos = new Vector2(ally.Pos.X, ally.Pos.Y);
                if (Vector2.Distance(myPos, allyPos) <= range) count++;
            }
            return count;
        }
        ...
        private void DrawCustomSkillUi(CustomSkillSetting skill, string label)
        {
            if (ImGui.TreeNode(label))
            {
                ...
                if (ImGui.Checkbox($"Enabled {label}", ref tmp)) skill.Enabled.Value = tmp;

                var newKey = ImGuiExtension.HotkeySelector($"Hotkey {label}: {skill.Key.Value}", skill.Key.Value);
                if (newKey != skill.Key.Value) skill.Key.Value = newKey;

                int tmpRange = skill.TriggerRange.Value;
                if (ImGui.SliderInt($"Range {label}", ref tmpRange, 0, 2000))
                    skill.TriggerRange.Value = tmpRange;

                int tmpMinE = skill.MinEnemy.Value;
                if (ImGui.SliderInt($"Min Enemy {label}", ref tmpMinE, 0, 20))
